...
2. On your own Mac, open **Signing & Capabilities** for the app and test targets.
   Enable **Automatically manage signing** and choose your own development team.
   The checked-in project uses the maintainer's signing certificate.
3. In **Build Settings**, set **Code Signing Identity** to **Apple Development**
   for both targets and build configurations when using automatic signing.
4. Select the **Concurred** scheme and **My Mac**, then press **⌘R**.
5. In the app, open **Settings** and save a provider API key.

After configuring signing, you can also build from the repository root:
...
The checked-in signing settings use the maintainer's Developer ID Application
certificate for team `75D25SJRM5`. Keep your signing identity consistent between builds; do not disable
...
On another developer's Mac, use that developer's signing team and certificate.
